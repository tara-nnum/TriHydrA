"""Compare dimensionless behaviour against hydrological analogue peers."""
...
class AnalogueContextResult:
    """Hydrological-analogue checks kept independently inspectable."""
...
def _event_metric_check(target, peers, metric, check, tolerance):
    ...
    for peer_id, peer in peers.items():
        ...
        # Express day-based agreement continuously while preserving the
        # configured rule: the tolerance corresponds to 0.50 similarity.
        similarity = float(np.clip(1.0 - difference / (2.0 * tolerance), 0.0, 1.0))
        rows.append({
