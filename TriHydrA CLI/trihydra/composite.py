...
def score_layer1(
    ...
    supplied = {} if config is None else dict(config).get("composite", {})
    settings = merge_config(DEFAULT_LAYER1_CONFIG["composite"], supplied)
    ...
    overall = (
        ...
        else "Needs review" if score >= int(classification["needs_review_minimum_score"])
        else "Minor concerns" if score >= int(classification["minor_concerns_minimum_score"])
...
def score_layer2_comparison(
    ...
    score = int(components["contribution"].sum())
    assessable_count = int(components["assessable"].sum())
    minimum_components = int(settings["minimum_assessable_components"])
    overall = (
        "Not assessable"
        if assessable_count < minimum_components
        else "Similar"
        if score <= int(settings["similar_score_maximum"])
        else "Review"
        if score <= int(settings["review_score_maximum"])
        else "Strong review"
    )
