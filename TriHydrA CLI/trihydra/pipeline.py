...
def run_station(
    ...
    *,
    ...
    if station.ml is not None and bool(layers["run_comparison"]):
        prepared = prepare_paired_comparison(
            ComparisonSeries(
                name="observation", values=station.obs,
                station_id=station.station_id, unit=station.unit,
                role="observation", provenance=station.obs_provenance,
                metadata=dict(station.metadata.get("observation_metadata", {})),
            ),
            ComparisonSeries(
                name=model_name, values=station.ml,
                station_id=station.station_id, unit=station.unit,
                role="simulation", provenance=station.ml_provenance,
                metadata=dict(station.metadata.get("model_metadata", {})),
            ),
        )
