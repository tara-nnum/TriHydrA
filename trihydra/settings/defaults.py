...
truths. Public TOML inputs are validated in ``settings.models``. Layers 1 and
...
DEFAULT_LAYER1_CONFIG: dict[str, dict[str, Any]] = {
    ...
    "composite": {
        ...
        "classification": {
            # Percentages are calculated against only enabled, assessable
            # checks, so a focused one-check run remains meaningful.
