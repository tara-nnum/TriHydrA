...
def station_availability(station: StationData) -> dict:
    """Summarise one station without changing its primary series."""
    series = station.series1
    valid = series.notna()
    valid_dates = series.index[valid]
    return {
        "station_id": station.station_id,
        "source_path": str(station.series1_provenance.path),
        "variable": station.series1_provenance.variable,
        "unit": station.unit,
        ...
        "first_valid_date": valid_dates.min() if len(valid_dates) else None,
        "last_valid_date": valid_dates.max() if len(valid_dates) else None,
    }
...
__all__ = ["pair_availability", "series_availability", "station_availability"]
