...
ComparisonMode = Literal["paired", "historical_profile", "independent_timespans"]
...
def prepare_independent_comparison(
    reference: ComparisonSeries,
    candidate: ComparisonSeries,
) -> PreparedComparison:
    """Prepare two independently selected timespans without date pairing."""
    _validate_pair(reference, candidate)
    reference_selected = reference.values.copy(deep=True)
    candidate_selected = candidate.values.copy(deep=True)
    if not reference_selected.notna().any():
        raise ValueError("series1 timespan contains no valid values.")
    if not candidate_selected.notna().any():
        raise ValueError("series2 timespan contains no valid values.")
    coverage = {
        **series_availability(reference_selected, "reference"),
        **series_availability(candidate_selected, "candidate"),
    ...
    return PreparedComparison(
        ...
        coverage=coverage,
        comparison_rules=(
            "The two configured timespans are assessed independently.",
            "Only distributions, signatures and event properties are compared.",
            "Daily paired metrics are not calculated.",
            "No values are imputed, clipped, aggregated or unit-converted.",
        ),
    )
