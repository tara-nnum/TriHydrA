"""Layer 2 diagnostic orchestration for one discharge series."""
...
from trihydra.settings.defaults import DEFAULT_LAYER2_CONFIG, merge_config
...
def run_layer2_diagnostics(
    ...
    *,
    ...
    settings = merge_config(DEFAULT_LAYER2_CONFIG, config)
    annual_settings = dict(settings["annual"])
    event_settings = dict(settings["events"])
    ...
    trigger_percentile = float(event_settings["trigger_percentile"])
    boundary_percentile = float(event_settings["boundary_percentile"])
    ...
    crosscheck = crosscheck_peak_outliers(
        ...
        minimum_event_duration_days=float(
            event_settings["spike_crosscheck_minimum_event_duration_days"]
        ),
    )
    ...
    if int(obs_series.isna().sum()) != missing_before:
        raise AssertionError("Layer 2 modified the supplied source series.")
    ...
    summary_metrics = {
        ...
        "median_annual_baseflow_index": diagnostic_values.get(
            "Median annual Lyne-Hollick baseflow index"
        ),
        "median_annual_seasonality_index": diagnostic_values.get(
            "Median annual Walsh-Lawler seasonality index"
        ),
